...
const STATUS_COLORS: Record<string, string> = {
  ...
  cancelled: 'bg-porcelain-deep text-ink-soft',
};
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="space-y-3">
        {requests.length === 0 ? (
          <div className="bg-white rounded-sm shadow-sm border border-line p-8 text-center text-ink-muted">
            Заявок пока нет
          </div>
        ) : (
          requests.map((req) => (
            <div
              ...
              className={`bg-white rounded-sm shadow-sm border cursor-pointer transition-all duration-200 ${
                selectedRequest?.id === req.id
                  ...
                  : 'border-line hover:border-pandora-rose/50'
              }`}
            >
              <div className="p-4">
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <div className="font-medium text-sm text-ink">{req.customerName}</div>
                    <div className="text-xs text-ink-muted">{req.customerPhone}</div>
                  </div>
                  <span className={`badge ${STATUS_COLORS[req.status] ?? 'bg-porcelain-deep text-ink-soft'} text-xs`}>
                    {STATUS_LABELS[req.status] ?? req.status}
                  </span>
                </div>
                <p className="text-sm text-ink-soft line-clamp-2 mb-2">{req.description}</p>
                ...
                {req.images.length > 0 && (
                  <div className="flex gap-1.5 mt-2">
                    ...
                    {req.images.length > 3 && (
                      <div className="w-10 h-10 bg-porcelain-deep rounded-sm flex items-center justify-center text-xs text-ink-muted">
                        +{req.images.length - 3}
                      </div>
                    )}
                  </div>
      ...
      {/* Detail */}
      {selectedRequest ? (
        <div className="bg-white rounded-sm shadow-sm border border-line p-6 space-y-5 h-fit sticky top-24">
          <div className="flex items-start justify-between">
            <h3 className="font-semibold text-ink">Заявка от {selectedRequest.customerName}</h3>
            <button onClick={() => setSelectedRequest(null)} className="text-ink-muted hover:text-ink-soft">
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <div className="text-sm space-y-2">
            <div><span className="text-ink-muted">Описание: </span>{selectedRequest.description}</div>
            {selectedRequest.budget && (
              <div><span className="text-ink-muted">Бюджет: </span>{selectedRequest.budget} с</div>
            )}
            {selectedRequest.preferredFlowers && (
              <div><span className="text-ink-muted">Цветы: </span>{selectedRequest.preferredFlowers}</div>
            )}
            {selectedRequest.preferredColors && (
              <div><span className="text-ink-muted">Цвета: </span>{selectedRequest.preferredColors}</div>
            )}
            {selectedRequest.deliveryDate && (
              <div><span className="text-ink-muted">Дата: </span>{selectedRequest.deliveryDate}</div>
            )}
            {selectedRequest.additionalNotes && (
              <div><span className="text-ink-muted">Примечания: </span>{selectedRequest.additionalNotes}</div>
            )}
          </div>
          ...
          {selectedRequest.images.length > 0 && (
            <div>
              <div className="text-xs text-ink-muted mb-2">Референсные фото:</div>
          ...
          {['pending', 'reviewing'].includes(selectedRequest.status) && (
            <div className="border-t border-line pt-4 space-y-3">
              <div className="text-sm font-medium text-ink-soft">Отправить предложение:</div>
              <input
                type="number"
                value={offerPrice}
        ...
        <div className="bg-white rounded-sm shadow-sm border border-line p-8 text-center text-ink-muted h-fit">
